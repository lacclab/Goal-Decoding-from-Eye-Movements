...
class HyperArgs(Tap):
    ...
    mailto: str = "shubi"  # Email to send notifications to.
...
def create_bash_scripts(hyper_args, sweep_ids):
    ...
    filename = base_path / f"sweep_{hyper_args.config_name}.sh"
    main_command = "; ".join(
        [
...
#SBATCH --mail-user={hyper_args.mailto}@campus.technion.ac.il
