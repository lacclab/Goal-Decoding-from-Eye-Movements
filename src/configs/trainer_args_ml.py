...
class BaseMLTrainerArgs:
    ...
    seed: int = 42
    devices: Any = 1  # Number of GPUs to use  #! set CUDA_VISIBLE_DEVICES to control which GPUs are used
    run_mode: RunModes = RunModes.TRAIN
    wandb_job_type: str = "MISSING"  # TODO CHANGE TO MISSING BETWEEN RUNS  # * Try to stick to 'debug', 'cv', single
    ...
    features_batch_size: int = 1024  # used for batch encoding for certain features
...
@register_config(group=GROUP)
@dataclass
class CfirMLVanila(BaseMLTrainerArgs):
    """
    Trainer arguments for the Cfir model.
    """
