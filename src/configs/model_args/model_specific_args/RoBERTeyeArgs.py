...
@register_config(group=GROUP)
@dataclass
class RoberteyeConcatCondPredWordsArgs(Roberteye):
    prediction_config: PredCfg = field(default_factory=ConditionPredCfg)
    use_fixation_report: bool = False
...
@register_config(group=GROUP)
@dataclass
class RoberteyeConcatCondPredFixationsArgs(RoberteyeFixation):
    prediction_config: PredCfg = field(default_factory=ConditionPredCfg)
    use_fixation_report: bool = True
    max_seq_len: int = 228  # in use only for creating TextDataset
    max_eye_len: int = 258
...
@register_config(group=GROUP)
@dataclass
class RoberteyeConcatCondPredFixationsFreezedArgs(RoberteyeConcatCondPredFixationsArgs):
    freeze: bool = True
