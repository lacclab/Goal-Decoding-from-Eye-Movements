...
@register_config(group=GROUP)
@dataclass
class BEyeLSTMCondPredArgs(BEyeLSTMArgs):
    prediction_config: PredCfg = field(
        default_factory=lambda: ConditionPredCfg(use_eyes_only=True)
    )

    model_params: BEyeLSTMParams = field(
        default_factory=lambda: BEyeLSTMParams(
            class_weights=None,
            item_level_features_modes=[
                ItemLevelFeaturesModes.DAVID,
            ],
        )
    )
...
@register_config(group=GROUP)
@dataclass
class FixSeqEncCondPredArgs(BEyeLSTMCondPredArgs):
    model_params: BEyeLSTMParams = field(
        default_factory=lambda: BEyeLSTMParams(
            class_weights=None,
            item_level_features_modes=[
                ItemLevelFeaturesModes.DAVID_EYES_ONLY,
            ],
