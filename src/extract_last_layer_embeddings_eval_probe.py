...
def main(cfg: Args) -> None:
    ...
    for fold_index in range(10):
        ...
        # Function to capture the output of the layer before the last layer
        embeddings = []

        def hook_fn(module, input, output):
            embeddings.append(output.last_hidden_state[:, 0, :])

        # Register the hook to the layer before the last layer (fc1 in this case)
        hook = model.model.roberta.register_forward_hook(hook_fn)

        # predict on eval dataloaders and test dataloaders
        # embeddings are captured by the hook
        model.eval()
        results = trainer.predict(model, dm)

        # Remove the hook
        hook.remove()

        # embeddings is a list of tensors shape (batch_size, embedding_dim).
        # the list length is the number of batches in the dataset.

        # results is a list of the number of eval_regime per eval_type={val, test}. 6 in total.
        # results[i] is a list of length the number of batches in the eval_type & eval_regime.
        # results[i][j] is a tuple of (labels, preds) where labels and preds are tensors of shape (batch_size, num_classes).
        ...
        for index, eval_type_results in enumerate(results):
            ...
            # embeddings
            n_batches = len(eval_type_results)
            embeddings_tensor = torch.cat(embeddings[:n_batches], dim=0)
            embeddings = embeddings[n_batches:]  # clear the used items from the list
        ...
        assert embeddings == [], "Not all embeddings were used!"
    ...
    filename = "trial_level_test_results_n_probe.csv"
    #! not general
    # res.to_csv(base_path / filename)
    # logger.info(f"Saved trial level results to {base_path / filename}")
    res.to_csv(f"/data/home/kfir-hadar/Cognitive-State-Decoding/{filename}")
    logger.info(
        f"Saved trial level results to /data/home/kfir-hadar/Cognitive-State-Decoding/{filename}"
    )
