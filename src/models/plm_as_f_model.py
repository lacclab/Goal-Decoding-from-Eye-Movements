...
def organise_sp_fixation_features(fixation_features, ia_features, scanpath):
    """
    Assumed order:
    ...
    features_by_sp_idx.append(fixation_features[..., 3])
    # 2. Total gaze duration (sum of all fixations on the word) (IA_DWELL_TIME) - ia_features
    features_by_word_idx.append(ia_features[..., 0])
    ...
    prev_sac_end_x = torch.zeros_like(fixation_features[..., 5]).to(
        fixation_features.device
    )
    prev_sac_end_x[..., 1:] = fixation_features[..., 5][..., :-1]
    prev_sac_start_x = torch.zeros_like(fixation_features[..., 6]).to(
        fixation_features.device
    )
    prev_sac_start_x[..., 1:] = fixation_features[..., 6][..., :-1]
    ...
    prev_sac_end_y = torch.zeros_like(fixation_features[..., 7]).to(
        fixation_features.device
    )
    prev_sac_end_y[..., 1:] = fixation_features[..., 7][..., :-1]
    prev_sac_start_y = torch.zeros_like(fixation_features[..., 8]).to(
        fixation_features.device
    )
    prev_sac_start_y[..., 1:] = fixation_features[..., 8][..., :-1]
    features_by_sp_idx.append(prev_sac_end_y - prev_sac_start_y)

    features_by_sp_idx, features_by_word_idx = (
        torch.stack(features_by_sp_idx, dim=2),
        torch.stack(features_by_word_idx, dim=2),
    )

    assert (
        features_by_sp_idx.shape[2] + features_by_word_idx.shape[2]
        == NUM_ADDITIONAL_FIXATIONS_FEATURES
    ), f"Number of features is not equal to {NUM_ADDITIONAL_FIXATIONS_FEATURES}"

    return features_by_sp_idx, features_by_word_idx
...
class PLMASFModel(BaseModel):
    ...
    def shared_step(
        ...
        features_by_sp_idx, features_by_word_idx = organise_sp_fixation_features(
            fixation_features=fixation_features,
            ia_features=batch_data.eyes,
            scanpath=scanpath,
        )
