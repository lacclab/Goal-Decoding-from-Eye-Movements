...
class FixationSeqEncoderModel(BaseModel):
    """
    Based on the BEyeLSTM model.
    Uses only the fixations block and the GSF block.
    """
    ...
    def __init__(
        ...
        # define which blocks to use #? move to model_args
        self.use_pos_block = False
        self.use_content_block = False
        self.use_fixations_block = True
        self.use_gsf_block = True
        self.num_blocks_used = sum(
            [
                self.use_pos_block,
                self.use_content_block,
                self.use_fixations_block,
                self.use_gsf_block,
            ]
        )
        assert self.num_blocks_used > 0, "No blocks used"

        if self.use_pos_block:
            self.pos_block = LSTMBlock(model_params, num_embed=model_params.num_pos)
        if self.use_content_block:
            self.content_block = LSTMBlock(
                model_params, num_embed=model_params.num_content
            )
        if self.use_fixations_block:
            self.fixations_block = LSTMBlock(
                model_params, input_dim=model_params.fixations_dim
            )
        if self.use_gsf_block:
            self.gsf_block = nn.Sequential(
                nn.Dropout(p=model_params.dropout_rate),
                nn.Linear(
                    in_features=model_params.gsf_dim,
                    out_features=model_params.gsf_out_dim,
                ),
                nn.ReLU(),
            )

        fc1_in_features = (
            model_params.lstm_block_fc2_out_dim
            * (self.num_blocks_used - self.use_gsf_block)
            + model_params.gsf_out_dim * self.use_gsf_block
        )  # -1 for gsf block
        self.fc1 = nn.Linear(
    ...
    def forward(  # type: ignore
        ...
        if self.use_fixations_block:
            concat_list.append(
                self.fixations_block(x_fixations, seq_lengths=seq_lengths)
            )
