...
class BaseMLModel:
    def __init__(self, model_args: BaseMLModelArgs, trainer_args: BaseMLTrainerArgs):
        ...
        self.use_item_level_features: bool = (
            len(model_args.model_params.item_level_features_modes) > 0
        )
        self.use_DWELL_TIME_WEIGHTED_PARAGRAPH_DOT_QUESTION_CLS_NO_CONTEXT: bool = model_args.model_params.use_DWELL_TIME_WEIGHTED_PARAGRAPH_DOT_QUESTION_CLS_NO_CONTEXT
        ...
        self.features_batch_size = trainer_args.features_batch_size
    ...
    def on_stage_end(self) -> None:
        ...
        wandb.run.summary[f"classless_accuracy/{self.stage}_average"] = np.mean(
            list(self.classless_accuracies.values())
        )  # type: ignore
        wandb.run.summary[  # type: ignore
            f"classless_accuracy/{self.stage}_weighted_average"
        ] = np.average(
            list(self.classless_accuracies.values()),
            weights=list(self.stage_count.values()),  # type: ignore
        )
        # Log balanced classless accuracy
        wandb.run.summary[f"balanced_classless_accuracy/{self.stage}_average"] = (
            np.mean(list(self.balanced_class_accuracies.values()))
        )  # type: ignore
        wandb.run.summary[  # type: ignore
        ...
        if self.use_DWELL_TIME_WEIGHTED_PARAGRAPH_DOT_QUESTION_CLS_NO_CONTEXT:
            features_list.append(
                feature_DWELL_TIME_WEIGHTED_PARAGRAPH_DOT_QUESTION_CLS_NO_CONTEXT(
                    ...
                    dwell_time_weights=train_batch.eyes[
                        ..., self.ia_features.index("IA_DWELL_TIME")
                    ],  # type: ignore
